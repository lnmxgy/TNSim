...
void init_indel_sim( seq_t *seq, mutseq_t *hap1, mutseq_t *hap2, indel_t *indelp)
{
	...
	while ( fscanf(indelfp, "%d %d %d %d %d %d %s\n", &idno, &idbeg, &idlen, &gtype, &idnum, &idvar, idstr) != EOF ){
		...
		if (idlen<0) {  // long delete from long indel file!!!
			...
			case 0: break;
			case 1: h = 0;
				indelmk = ret[h]->s[idbeg-1]& DELETE;
				ret[h]->s[idbeg-1] = (ret[h]->s[idbeg-1] & 0x03 );
				if (indelmk == DELETE) ret[h]->s[idbeg-1] |= (LONGINSERT|GTYPEAB|(addnum<<8));
				else ret[h]->s[idbeg-1] |= (LONGINSERT|INSERT|GTYPEAB|(addnum<<8));
				break;
			case 2: h = 1;
				indelmk = ret[h]->s[idbeg-1]& DELETE;
				ret[h]->s[idbeg-1] = (ret[h]->s[idbeg-1] & 0x03 );
			...
			case 3: h = 0;
				indelmk = ret[h]->s[idbeg-1]& DELETE;
				ret[h]->s[idbeg-1] = (ret[h]->s[idbeg-1] & 0x03 );
				if (indelmk == DELETE) ret[h]->s[idbeg-1] |= (LONGINSERT|GTYPEBB|(addnum<<8));
				else ret[h]->s[idbeg-1] |= (LONGINSERT|INSERT|GTYPEBB|(addnum<<8));
				h = 1;
				indelmk = ret[h]->s[idbeg-1]& DELETE;
				ret[h]->s[idbeg-1] = (ret[h]->s[idbeg-1] & 0x03 );
				if (indelmk == DELETE) ret[h]->s[idbeg-1] |= (LONGINSERT|GTYPEBB|(addnum<<8));
